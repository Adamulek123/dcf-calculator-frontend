...
const CACHE_POLICIES = Object.freeze({
    ...
    earningsManifest: policy({
        ...
        maxBytes: 12 * 1024 * 1024,
        version: 2,
        invalidatedBy: ["manual-refresh", "schema-version-change"],
        key: () => "earnings-manifest:v2",
    }),
    earningsWeek: policy({
        ownership: "public",
        ttlMs: HOUR,
        ...
        maxBytes: 12 * 1024 * 1024,
        version: 2,
        invalidatedBy: ["manual-refresh", "manifest-revision-change", "schema-version-change"],
        key: ({ weekStart, weekRevision }) => `earnings-week:v2:${encodeURIComponent(String(weekStart || ""))}:${encodeURIComponent(String(weekRevision || ""))}`,
    }),
    earningsEstimate: policy({
        ownership: "public",
        ttlMs: HOUR,
        staleTtlMs: 7 * DAY,
        maxEntries: 200,
        ...
        version: 1,
        invalidatedBy: ["manual-refresh", "manifest-revision-change", "schema-version-change"],
        key: ({ eventId, weekRevision }) => `earnings-estimate:v1:${encodeURIComponent(String(eventId || ""))}:${encodeURIComponent(String(weekRevision || ""))}`,
    }),
